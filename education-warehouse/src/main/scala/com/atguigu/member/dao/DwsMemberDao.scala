...
object DwsMemberDao {
  ...
  def queryAppregurlCount(sparkSession: SparkSession, dt: String) = {
    sparkSession.sql(s"select appregurl,count(uid),dt,dn from dws.dws_member where dt='${dt}' group by appregurl,dn,dt")
  }

  //统计所属网站人数
  def querySiteNameCount(sparkSession: SparkSession, dt: String) = {
    sparkSession.sql(s"select sitename,count(uid),dt,dn from dws.dws_member where dt='${dt}' group by sitename,dn,dt")
  }

  //统计所属来源人数
  def queryRegsourceNameCount(sparkSession: SparkSession, dt: String) = {
    sparkSession.sql(s"select regsourcename,count(uid),dt,dn from dws.dws_member where dt='${dt}' group by regsourcename,dn,dt ")
  }

  //统计通过各广告注册的人数
  def queryAdNameCount(sparkSession: SparkSession, dt: String) = {
    sparkSession.sql(s"select adname,count(uid),dt,dn from dws.dws_member where dt='${dt}' group by adname,dn,dt")
  }

  //统计各用户等级人数
  def queryMemberLevelCount(sparkSession: SparkSession, dt: String) = {
    sparkSession.sql(s"select memberlevel,count(uid),dt,dn from dws.dws_member where dt='${dt}' group by memberlevel,dn,dt")
  }

  //统计各用户vip等级人数
  def queryVipLevelCount(sparkSession: SparkSession, dt: String) = {
    sparkSession.sql(s"select vip_level,count(uid),dt,dn from dws.dws_member group where dt='${dt}' group by vip_level,dn,dt")
  }

  //统计各支memberlevel等级 付金额前三的用户
  def getTop3MemberLevelPayMoneyUser(sparkSession: SparkSession, dt: String) = {
//    sparkSession.sql("select uid,memberlevel,register,appregurl,regsourcename,adname,sitename,vip_level," +
//      "paymoney,rownum,dt,dn from(select uid,ad_id,memberlevel,register,appregurl,regsource" +
//      ",regsourcename,adname,siteid,sitename,vip_level,cast(paymoney as decimal(10,4)) as paymoney,row_number() over" +
...
//      " where rownum<4 order by memberlevel,rownum")
    ...
    sparkSession.sql(
      s"""
         ...
         |            dn,
         |            dt
         |      from dws.dws_member where dt='${dt}'
         |)t1
         |where rownum<4 order by memberlevel,rownum
       """.stripMargin)
  }
