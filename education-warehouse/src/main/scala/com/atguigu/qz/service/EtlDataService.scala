...
import com.atguigu.qz.util.ParseJsonData
...
object EtlDataService {
  ...
  /**
    ...
    * @param sparkSession
    */
  def etlQzChapter(sc: SparkContext, sparkSession: SparkSession): Unit ={
    import sparkSession.implicits._

    sc.textFile("/user/atguigu/ods/QzChapter.log").filter(item=>{
      val obj: JSONObject = JSON.parseObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partition=>{
        partition.map{item=>
          val qzChapter: DwdQzChapter = JSON.parseObject(item, classOf[DwdQzChapter])
          qzChapter
        }
  ...
  /**
    ...
    * @param sparkSession
    */
  def etlQzChapterList(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/atguigu/ods/QzChapterList.log").filter(item=>{
      val obj: JSONObject = JSON.parseObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partition=>{
      partition.map{item=>
        val qzChapterList: DwdQzChapterList = JSON.parseObject(item, classOf[DwdQzChapterList])
        qzChapterList
      }
...
//    * @param sparkSession
//    */
//  def etlQzPoint(sc: SparkContext, sparkSession: SparkSession) = {
//    import sparkSession.implicits._
//    sc.textFile("/user/atguigu/ods/QzPoint.log").filter(item=>{
//      val obj: JSONObject = JSON.parseObject(item)
//      obj.isInstanceOf[JSONObject]
//    }).mapPartitions(partition=>{
//      partition.map{item=>
//        val qzPoint: DwdQzPoint = JSON.parseObject(item, classOf[DwdQzPoint])
//        qzPoint.score = BigDecimal(qzPoint.score.toDouble).setScale(1, BigDecimal.RoundingMode.HALF_UP) // 保留一位小数，并四舍五入
//        qzPoint
//      }
...
//  }
  ...
  /**
    ...
    * @return
    */
  def etlQzPointQuestion(sc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    sc.textFile("/user/atguigu/ods/QzPointQuestion.log").filter(item => {
      val obj: JSONObject = JSON.parseObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzSiteCourse(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("/user/atguigu/ods/QzSiteCourse.log").filter(item => {
      ...
      partitions.map(item => {
        ...
        val dt = jsonObject.getString("dt")
        val dn = jsonObject.getString("dn")
        (sitecourseid, siteid, courseid, sitecoursename, coursechapter, sequence, status, creator, createtime, helppaperstatus, servertype, boardid, showstatus, dt, dn)
      })
