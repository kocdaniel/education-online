...
//    dwdQzChapter.join(dwdQzChapterlist, dwdQzChapter("chapterlistid") === dwdQzChapterlist("chapterlistid") && dwdQzChapter("dn") === dwdQzChapterlist("dn"))
    val joinResult: DataFrame = dwdQzChapter.join(dwdQzChapterlist, Seq("chapterlistid", "dn"))
      .join(dwdQzPoint, Seq("chapterid", "dn"))
      .join(dwdQzPointQuestion, Seq("pointid", "dn"))

    joinResult.select("chapterid", "chapterlistid", "chaptername", "sequence", "showstatus", "status",
  ...
  def saveDwsQzCourse(sparkSession: SparkSession, dt: String) = {
    ...
    val joinResult = dwdQzSiteCourse.join(dwdQzCourse, Seq("courseid", "dn"))
      ...
      .select("sitecourseid", "siteid", "courseid", "sitecoursename", "coursechapter",
        ...
        , "dt", "dn")
    joinResult.coalesce(1).write.mode(SaveMode.Append).insertInto("dws.dws_qz_course")
  ...
  def saveDwsQzMajor(sparkSession: SparkSession, dt: String) = {
    ...
    val joinResult = dwdQzMajor.join(dwdQzWebsite, Seq("siteid", "dn"))
    ...
    joinResult.coalesce(1).write.mode(SaveMode.Append).insertInto("dws.dws_qz_major")
  ...
  def saveDwsQzPaper(sparkSession: SparkSession, dt: String) = {
    ...
    val joinResult = dwdQzPaperView.join(dwdQzCenterPaper, Seq("paperviewid", "dn"), "left")
    ...
    joinResult.coalesce(1).write.mode(SaveMode.Append).insertInto("dws.dws_qz_paper")
  ...
  def saveDwsQzQuestionTpe(sparkSession: SparkSession, dt: String) = {
    ...
    val joinResult = dwdQzQuestion.join(dwdQzQuestionType, Seq("questypeid", "dn"))
    ...
    joinResult.coalesce(1).write.mode(SaveMode.Append).insertInto("dws.dws_qz_question")

  }

  // 用户试卷详情大宽表
//  def saveDwsUserPaperDetail(sparkSession: SparkSession, dt: String) = {
//    val dwdQzMemberPaperQuestion = UserPaperDetailDao.getDwdQzMemberPaperQuestion(sparkSession, dt).drop("paperid") // drop相同的字段名，避免select几个相同的字段名
//      .withColumnRenamed("question_answer", "user_question_answer") // 对字段进行重命名
//
...
//        "question_splitscoretype", "user_question_answer", "dt", "dn").coalesce(3)
  ...
  def saveDwsUserPaperDetail(sparkSession: SparkSession, dt: String) = {
    ...
    dwdQzMemberPaperQuestion.join(dwsQzCourse, Seq("sitecourseid", "dn")).
      ...
      .select("userid", "courseid", "questionid", "useranswer", "istrue", "lasttime", "opertype",
        ...
        "splitscore", "lecture", "question_creator", "question_createtime", "question_modifystatus", "question_attanswer",
        "question_questag", "question_vanalysisaddr", "question_difficulty", "quesskill", "vdeoaddr", "question_description",
        "question_splitscoretype", "user_question_answer", "dt", "dn").coalesce(4)
